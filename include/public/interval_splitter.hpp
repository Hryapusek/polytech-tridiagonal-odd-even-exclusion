...
auto calc_h(const std::vector<double>& intervals, size_t index) -> double;

// Calculate the cross h of an interval
auto calc_cross_h(const std::vector<double>& intervals, size_t index) -> double;

/// @return middle point between `index` and `index - 1`
auto middle_point(const std::vector<double>& intervals, size_t index) -> double;
